...
const ManageAllergenPage = () => {
  ...
  const addFeaturesModalShow = useSelector(
    (state) => state.features.showAddFeaturesModalData
  );
  const updateFeaturesModalShow = useSelector(
    (state) => state.features.showUpdateFeaturesModalData
  );
  const deleteModalShow = useSelector(
    (state) => state.features.showDeleteFeaturesModalData
  );
  ...
  useEffect(() => {
    setMypage(1);
    dispatch(
      getAllFeaturesData({ start: 0, length: perPage, search: inputValue })
    );
      ...
      cell: (row) => (
        <CDropdown className="btn-group">
          ...
          <CDropdownMenu placement="left">
            ...
            <CDropdownItem
              onClick={() => {
                dispatch(showDeleteFeaturesModal(true));
                setSelectedId(row._id);
                seImagePath(row.image);
              }}
            >
    ...
    <>
          ...
          <CCard>
            ...
            <CCardBody>
              <CRow className="justify-content-between align-items-center ">
                ...
                <div>
                  <AddFeaturesComponent
                    show={addFeaturesModalShow}
                    onClose={() => dispatch(showAddFeaturesModal(false))}
                    perpage={perPage}
                    mypage={myPage}
              ...
              {isLoading ? (
                ...
                <React.Fragment>
                  {features_Data.restaurantFeatureList &&
                  features_Data.restaurantFeatureList.length > 0 ? (
                    <CCard>
                      <DataTable
                        columns={columns}
      ...
      <React.Fragment>
        <UpdateFeaturesComponent
          show={updateFeaturesModalShow}
          onClose={() => dispatch(showUpdateFeaturesModal(false))}
      ...
      <React.Fragment>
        <DeleteFeaturesComponent
          show={deleteModalShow}
          onClose={() => dispatch(showDeleteFeaturesModal(false))}
          selectedid={selectedId}
          imagepath={imagePath}
          perpage={perPage}
          mypage={myPage}
