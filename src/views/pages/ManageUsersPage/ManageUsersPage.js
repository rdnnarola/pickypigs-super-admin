...
const ManageUsersPage = () => {
  ...
  const updateUserPasswordModalShow = useSelector(
    (state) => state.users.showUpdateUserPasswordModalData
  );
  ...
  useEffect(() => {
    setMypage(1);
    dispatch(
      getAllUsersData({ start: 0, length: perPage, search: inputValue })
    );
  ...
  const columns = [
    ...
    {
      selector: "user_preferenceDetail.phone",
      name: "Mobile",
      cell: (row) => <span>{row.user_preferenceDetail.phone}</span>,
    },
    {
      selector: "updatedAt",
      name: "Updated At",
      ...
      cell: (row) => (
        <CDropdown className="btn-group">
          ...
          <CDropdownMenu placement="left">
            <CDropdownItem
              onClick={() => {
                dispatch(showUpdateUserPasswordModal(true));
                setSelectedId(row._id);
                setSelectedMail(row.user_preferenceDetail.name);
  ...
  const handleDownloadScv = () => {
    ...
    Axios.post(dataURL, responseType)
      .then((response) => {
    ...
    <>
            ...
            <CCardBody>
              <CRow className="justify-content-between align-items-center ">
                ...
                <CCol className="mb-4 d-flex justify-content-end" sm="8">
                  ...
                  <CButton
                    className="btn pinkline-btn text-uppercase rounded-pill ml-3"
                    onClick={handleDownloadScv}
                  >
                    <CIcon
      ...
      <React.Fragment>
        <UpdateUserPasswordModalComp
          show={updateUserPasswordModalShow}
          onClose={() => dispatch(showUpdateUserPasswordModal(false))}
